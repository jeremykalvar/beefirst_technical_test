...
@pytest.mark.asyncio
async def test_users_repo_create_or_update_and_get_with_hash(pool):
    """
    ...
    NOTE: we use the shared 'pool' fixture (session-scoped MiniPool) so we don't
    open/close the global psycopg pool here (avoids cross-loop close errors).
    """
